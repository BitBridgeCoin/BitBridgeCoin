...
*Setup instructions for a Gitian build of BitBridgeCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitBridgeCoin
...
- [Building BitBridgeCoin Core](#building-bitbridgecoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitbridgecoin-host/README.md).
...
Clone the git repositories for bitbridgecoin and Gitian.
...
in the bitbridgecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BitBridgeCoin Core
----------------

To build BitBridgeCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitbridgecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitbridgecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitbridgecoin/bitbridgecoin
...
For example:
```bash
URL=https://github.com/laanwj/bitbridgecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitbridgecoin=${COMMIT} --url bitbridgecoin=${URL} ../bitbridgecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitbridgecoin=${COMMIT} --url bitbridgecoin=${URL} ../bitbridgecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitbridgecoin=${COMMIT} --url bitbridgecoin=${URL} ../bitbridgecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitbridgecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitbridgecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitbridgecoin
SIGPATH=/some/root/path/bitbridgecoin-detached-sigs

./bin/gbuild --url bitbridgecoin=${BTCPATH},signature=${SIGPATH} ../bitbridgecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitbridgecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitbridgecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitbridgecoin-osx-build.assert
```
...
[bitbridgecoin-core/gitian.sigs](https://github.com/bitbridgecoin-core/gitian.sigs/) repository, or if that's not possible create a pull
