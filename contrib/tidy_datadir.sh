#!/bin/bash
# Copyright (c) 2013 The BitBridgeCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BitBridgeCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BitBridgeCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BitBridgeCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BitBridgeCoin 0.7 datadir."
    ;;
  3)
    echo "Detected BitBridgeCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected BitBridgeCoin 0.8 datadir."
    ;;
esac
