...
The RPM spec file provided here is for BitBridgeCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitbridgecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitBridgeCoin git repository](https://github.com/bitbridgecoin/bitbridgecoin) but are not part of
...
Currently only `Source10` is used, for the example `bitbridgecoin.conf` file.
...
in the bitbridgecoin git repository. At present only an SVG version of the BitBridgeCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitbridgecoin
...
needed when 0.12.1 is released, a proper fix is already in the BitBridgeCoin git
master branch.
...
idea to build BitBridgeCoin Core against a static version of that library compiled
...
The `bitbridgecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitbridgecoin-server` package. At this point, most RPM based
...
The `bitbridgecoin-server` package should have SELinux support. How to properly do
...
BitBridgeCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitbridgecoin/). He has been
packaging BitBridgeCoin for Fedora at least since 2012.
...
4. I renamed the `bitbridgecoin` package that contains the Qt GUI to `bitbridgecoin-core` as
that appears to be how the general population refers to it, in contrast to
`bitbridgecoin-xt` or `bitbridgecoin-classic`. I wanted to make sure the general population
