...
###   bitcoincore.org and bitbridgecoin.org and compares them.
...
WORKINGDIR="/tmp/bitbridgecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoincore.org"
HOST2="https://bitbridgecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitbridgecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitbridgecoin-',
...
if [ $? -ne 0 ]; then
   echo "bitbridgecoin.org failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitbridgecoin.org and bitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitBridgeCoin Core binary release signing key installed?"
   fi
