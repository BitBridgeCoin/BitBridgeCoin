...
#if defined(NDEBUG)
# error "BitBridgeCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "BitBridgeCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    //     return 0;
    // if (nHeight == 1)
    //   CAmount nSubsidy = 21000000000 * COIN;
    // else
    if (nHeight > 420000)
      return 0;
...
void ThreadScriptCheck() {
    RenameThread("bitbridgecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    if (block.vtx[0]->GetValueOut() > nFees + GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus()) )
        return state.DoS(100,
                         error("ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d)",
                               block.vtx[0]->GetValueOut(), blockReward),
                               REJECT_INVALID, "bad-cb-amount-coinbase");
   if (block.vtx[0]->vout[0].nValue  > nFees + (GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus())/10) )
       return state.DoS(100,
                        error("ConnectBlock(): vout[0]",
                              block.vtx[0]->GetValueOut(), blockReward),
                              REJECT_INVALID, "bad-cb-amount-vout0");
   ...
   if (block.vtx[0]->vout[1].scriptPubKey != mnPayee)
     return state.DoS(100,
                    error("Incorrect vout[1] address"),
                          REJECT_INVALID, "bad-cb-amount-missing");
    if (block.vtx[0]->vout[1].nValue != (9 * GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus()) / 10))
       return state.DoS(100,
                      error("ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d) to genesis address",
                                  block.vtx[0]->vout[1].nValue, blockReward),
                                  REJECT_INVALID, "bad-cb-amount-genesis");
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    //       return state.DoS(100, false, REJECT_INVALID, "bad-cb-master-share-missing", false, "coinbase missing master tx");
    // //
    //   if (block.vtx[0]->vout.size() > 1 && block.vtx[0]->vout[1].scriptPubKey != mnPayee)
    //           return state.DoS(100, false, REJECT_INVALID, "bad-cb-master-share-incorrect", false, "coinbase incorrect master tx");
    //   if (block.vtx[0]->vout[1].nValue != (450000 * COIN))
    //           return state.DoS(100, false, REJECT_INVALID, "bad-cb-master-share-incorrect", false, "coinbase incorrect master tx value");
    //
    // }
